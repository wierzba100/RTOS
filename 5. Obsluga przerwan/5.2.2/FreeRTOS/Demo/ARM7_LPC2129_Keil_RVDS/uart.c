...
{
	char cOdebranyznak;
	if(xQueueReceive( xReceiverQueue, &cOdebranyznak, 10) == pdPASS)
	{
...
__irq void UART0_Interrupt (void) {
   ...
   {
      cOdebranyZnak = U0RBR;
			xQueueSend(xReceiverQueue, &cOdebranyZnak, portMAX_DELAY);
...
void UART_InitWithInt(unsigned int uiBaudRate){
	...
	xReceiverQueue = xQueueCreate(RECEIVER_SIZE, sizeof (char));
