	...
	while(1)
	{
		if(xQueueReceive( xQueueEvents, &buffer, 10) == pdPASS)
		{
			...
			}else if(eCompareString(buffer,"callib") == EQUAL)
			{
				ServoCallib();
				Uart_PutString("ok");
			}else if(eCompareString(buffer,"state") == EQUAL)
			{
				sServoStatus = Servo_State();
				switch (sServoStatus.eState)
				{
					case CALLIB: CopyString("state callib ",buffer); break; 
					case IDLE : CopyString("state iddle ",buffer); break;
					case IN_PROGRESS : CopyString("state in_proggres ",buffer); break; 
					//case _WAITING : CopyString("state waiting ",cString); break;
					default:break; 
				};
				AppendUIntToString(sServoStatus.uiCurrentPosition,buffer);
				AppendString("\n",buffer);
